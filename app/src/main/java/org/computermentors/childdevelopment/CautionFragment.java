...
public class CautionFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        LinearLayout linearLayout = view.findViewById(R.id.cautionLayout);
        String[] cautions = Milestones.concerns[index].split("`");
        checkBoxes = new CheckBox[cautions.length];
        boolean[] checkedBoxes = new boolean[checkBoxes.length];
        if (savedInstanceState != null && savedInstanceState.getBooleanArray(KEY_CHECKED_BOXES) != null){
            checkedBoxes = savedInstanceState.getBooleanArray(KEY_CHECKED_BOXES);
        }
        setUpCheckBoxes(cautions, linearLayout, checkedBoxes);


        return view;
    }

    private void setUpCheckBoxes(String[] cautions, ViewGroup container, boolean[] checkedBoxes){
        int i = 0;
